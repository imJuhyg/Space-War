...
buttons = [29, 31, 33, 32, 35, 37]  # left, up, right, down, a, b
...
def run():
    ...
    while not sysOn:
        ...
        # 버튼 입력으로 제어하기
        # left
        buttonState[0] = GPIO.input(buttons[0])
        if buttonState[0] == 1:
            spaceshipLeft -= 0.1
        elif buttonState[0] == 0:
            spaceshipLeft = 0

        # right
        buttonState[2] = GPIO.input(buttons[2])
        if buttonState[2] == 1:
            spaceshipRight += 0.1
        elif buttonState[2] == 0:
            spaceshipRight = 0

        # up
        buttonState[1] = GPIO.input(buttons[1])
        if buttonState[1] == 1:
            spaceshipUp -= 0.1
        elif buttonState[1] == 0:
            spaceshipUp = 0

        # down
        buttonState[3] = GPIO.input(buttons[3])
        if buttonState[3] == 1:
            spaceshipDown += 0.1
        elif buttonState[3] == 0:
            spaceshipDown = 0
        ...
        # b
        buttonState[5] = GPIO.input(buttons[5])
        if buttonState[5] == 1:
            # 핵미사일 발사
            if nuclearCount > 0:
        ...
        # spaceship 좌표조정
        x += spaceshipLeft + spaceshipRight
        y += spaceshipUp + spaceshipDown
...
def gameStart():
    ...
    while True:
        ...
        if resultTime > 1:
            # 아무키나 입력 받음
            for i in range(len(buttons)):
                buttonState[i] = GPIO.input(buttons[i])
